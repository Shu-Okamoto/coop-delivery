...
import { apiGet, memberPost, getMemberSession } from '@/lib/api';
...
export default function NewDraftPage() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await memberPost('/api/schedules/draft', {
        name: name.trim(),
  ...
  return (
    <div className="driver-body">
      ...
      <main className="driver-main">
        ...
        <div className="card">
          <label style={{ display: 'block', marginBottom: 8 }}>ルート名 *
            <input value={name} onChange={(e) => setName(e.target.value)}
                   placeholder="例: 午前便（岩国→周南）"
                   style={{ width: '100%', padding: 8, marginTop: 4, border: '1px solid #d5dbdb', borderRadius: 6 }} />
          </label>
          <label style={{ display: 'block' }}>備考
