-- ============================================================
-- 新しい Supabase プロジェクトへ coop-delivery を移行
-- 使い方: 新プロジェクトの SQL Editor に「全文をまとめて」貼り付けて Run
--   ※ psql も 7-Zip も不要。ブラウザだけで完結します。
--   ※ 新規プロジェクト前提。public スキーマに作成するのでアプリは
--      SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY の差し替えだけで動きます。
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_positions_route ON public.vehicle_positions(route_id, recorded_at DESC);

-- ===== 2. データ投入（親→子の順） =====

INSERT INTO public.members (id,code,name,type,address,lat,lng,contact_name,phone,email,created_at) VALUES
...
INSERT INTO public.vehicles (id,code,name,plate_number,vehicle_type,capacity_kg,refrigerated,active,created_at) VALUES
...
INSERT INTO public.drivers (id,member_id,name,phone,pin_code,active,created_at) VALUES
...
INSERT INTO public.routes (id,route_code,name,scheduled_date,driver_id,vehicle_id,start_time,end_time,planned_start_time,status,notes,created_at,updated_at) VALUES
...
INSERT INTO public.route_stops (id,route_id,stop_order,stop_type,member_id,address,lat,lng,cargo_description,weight_kg,refrigerated,scheduled_time,arrived_at,completed_at,completed,notes,photo_url,created_at) VALUES
...
INSERT INTO public.vehicle_positions (id,route_id,vehicle_id,driver_id,lat,lng,heading,speed_kmh,recorded_at) VALUES
...
SELECT setval('public.routes_id_seq',            (SELECT MAX(id) FROM public.routes));
SELECT setval('public.route_stops_id_seq',       (SELECT MAX(id) FROM public.route_stops));
SELECT setval('public.vehicle_positions_id_seq', (SELECT MAX(id) FROM public.vehicle_positions));

-- ===== 4. 最新位置ビュー（security_invoker 付き） =====
CREATE OR REPLACE VIEW public.vehicle_latest_positions
...
-- ===== 6. RLS（service_role 全許可。API は service_role キーで接続） =====
...
DO $$ BEGIN CREATE POLICY "service_role full" ON public.members           FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "service_role full" ON public.vehicles          FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "service_role full" ON public.drivers           FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "service_role full" ON public.routes            FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "service_role full" ON public.route_stops       FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "service_role full" ON public.vehicle_positions FOR ALL TO service_role USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
UNION ALL SELECT 'vehicle_positions', count(*) FROM public.vehicle_positions;
